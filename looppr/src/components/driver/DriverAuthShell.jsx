...
export default function DriverAuthShell({ title, subtitle, children, footer, description, keywords, noindex = true, jsonLd }) {
  return (
    <div className="relative grid min-h-screen lg:grid-cols-2">
      <SEO title={`${title} · Drive with Looppr`} description={description} keywords={keywords} noindex={noindex} jsonLd={jsonLd} />
      ...
      <Link to="/drive" className="absolute left-6 top-6 z-10 flex items-center gap-0 lg:hidden">
        ...
        <span className="ml-2 rounded-full bg-sky-100 px-2.5 py-0.5 text-xs font-semibold text-sky-700">Drive</span>
      </Link>
      ...
      <div className="relative hidden overflow-hidden bg-ink lg:flex lg:items-center lg:justify-center lg:p-16">
        <div
          ...
          style={{
            backgroundImage:
              'radial-gradient(circle at 15% 15%, rgba(56,189,248,0.28), transparent 45%), radial-gradient(circle at 85% 75%, rgba(251,230,194,0.22), transparent 42%)',
          }}
        />

        <div className="relative z-10 flex max-w-lg flex-col items-center text-center">
          <Link to="/drive" className="flex items-center gap-0">
          ...
          <p className="mt-12 font-display text-5xl font-bold leading-[1.05] tracking-tight text-white xl:text-6xl">
            Drive when you want, <em className="italic text-sky-300">earn what you need.</em>
          </p>

          <p className="mt-7 max-w-md text-lg leading-relaxed text-white/70 xl:text-xl">
            Pick up laundry, drop it at a partner laundromat, deliver it back clean. Short local
            routes on your own schedule.
          </p>
      ...
      <div className="flex items-center justify-center px-4 py-16 sm:px-8 lg:px-12">
        <div className="w-full max-w-lg">
          <p className="text-center text-sm font-bold uppercase tracking-[0.18em] text-sky-600">For drivers</p>
