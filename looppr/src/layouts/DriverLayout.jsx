...
const NAV_ITEMS = [
  { to: '/drive/dashboard', end: true, label: 'Dashboard', icon: <><rect x="3" y="3" width="7" height="7" rx="1.5" /><rect x="14" y="3" width="7" height="7" rx="1.5" /><rect x="3" y="14" width="7" height="7" rx="1.5" /><rect x="14" y="14" width="7" height="7" rx="1.5" /></> },
  { to: '/drive/dashboard/incoming', label: 'Incoming Deliveries', icon: <><path d="M4 4h16v5H4zM4 13h16v7H4z" /><path d="M8 16h5" /></> },
  { to: '/drive/dashboard/deliveries', label: 'My Deliveries', icon: <><path d="M5 17h14l-1.5-6.5A2 2 0 0 0 15.6 9H8.4a2 2 0 0 0-1.9 1.5z" /><circle cx="7.5" cy="17.5" r="1.5" /><circle cx="16.5" cy="17.5" r="1.5" /></> },
  { to: '/drive/dashboard/earnings', label: 'Earnings', icon: <><path d="M12 1v22M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6" /></> },
  { to: '/drive/dashboard/notifications', label: 'Notifications', icon: <><path d="M18 8a6 6 0 1 0-12 0c0 7-3 9-3 9h18s-3-2-3-9" /><path d="M13.7 21a2 2 0 0 1-3.4 0" /></> },
  { to: '/drive/dashboard/profile', label: 'Profile', icon: <><path d="M3 21V9l9-6 9 6v12" /><path d="M9 21v-6h6v6" /></> },
  { to: '/drive/dashboard/settings', label: 'Settings', icon: <><circle cx="12" cy="12" r="3" /><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-2.82 1.17V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 7 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 2.6 15H2.5a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06A1.65 1.65 0 0 0 9 4.6h.09A1.65 1.65 0 0 0 11 2.5a2 2 0 0 1 4 0v.09A1.65 1.65 0 0 0 17 4.6a1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 21.4 9v.09a2 2 0 0 1 0 4z" /></> },
]

function SidebarNav({ driver, onNavigate, onLogout }) {
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-0 px-5 py-5">
        ...
        <span className="ml-2 rounded-full bg-sky-100 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-sky-700">Driver</span>
      </div>
      ...
      <div className="border-t border-line px-3 py-3">
        <div className="flex items-center gap-3 rounded-xl px-3 py-2">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-xs text-ink/50">{driver?.email}</p>
            {driver?.vehicleName && <p className="truncate text-xs text-ink/40">{driver.vehicleName}</p>}
          </div>
        </div>
...
// Online/Offline toggle — going online also starts sharing live GPS location
// (see useDriverLocation) so the backend can populate DriverUser.location for
// the future proximity-based assignment engine.
function AvailabilityToggle() {
  ...
  const { permission } = useDriverLocation(online)
  ...
  return (
    <div className="flex items-center gap-2">
      {online && permission === 'denied' && (
        <span className="hidden text-xs font-medium text-amber-600 sm:inline" title="Enable location access in your browser so nearby deliveries can find you">
          Location off
        </span>
      )}
      <button
  ...
  return (
    <div className="min-h-screen bg-linen">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-64 border-r border-line bg-white lg:block">
        <SidebarNav driver={driver} onLogout={handleLogout} />
      </aside>
      ...
      {drawerOpen && (
        <div className="fixed inset-0 z-40 lg:hidden">
          ...
          <aside className="absolute inset-y-0 left-0 w-64 bg-white shadow-xl">
            <SidebarNav driver={driver} onLogout={handleLogout} onNavigate={() => setDrawerOpen(false)} />
          </aside>
        </div>
      )}
        ...
        <header className="sticky top-0 z-20 border-b border-line bg-white/80 backdrop-blur">
          <div className="flex items-center justify-between gap-3 px-4 py-3 sm:px-6">
            ...
            <p className="hidden text-sm font-medium text-ink/50 sm:block">
              Welcome, <span className="font-semibold text-ink">{driver?.name}</span>
            </p>
