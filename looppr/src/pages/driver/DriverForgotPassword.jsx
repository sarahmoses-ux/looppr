...
export default function DriverForgotPassword() {
  const { forgotPassword, resetPassword } = useDriverAuth()
  ...
  async function handleRequest(e) {
    e.preventDefault()
    if (!/^\S+@\S+\.\S+$/.test(form.email)) return setErrors({ email: 'Enter your email address.' })
    ...
    >
      {step === 'request' ? (
        <form onSubmit={handleRequest} noValidate className="space-y-5">
          <Input id="email" name="email" type="email" label="Email" value={form.email} onChange={handleChange} error={errors.email} placeholder="you@example.com" />
