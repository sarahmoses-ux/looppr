...
export default function PartnerForgotPassword() {
  const { forgotPassword, resetPassword } = usePartnerAuth()
  ...
  async function handleRequest(e) {
    e.preventDefault()
    if (!/^\S+@\S+\.\S+$/.test(form.email)) return setErrors({ email: 'Enter your business email.' })
    setErrors({}); setFormError(''); setStatus('pending')
    try {
      await forgotPassword(form.email)
      setStep('reset'); setStatus('idle')
    } catch {
      setFormError('Could not send a code right now. Please try again.'); setStatus('idle')
    }
  }
  ...
  async function handleReset(e) {
    ...
    if (!/^(?=.*[A-Za-z])(?=.*\d).{8,}$/.test(form.newPassword)) next.newPassword = 'At least 8 characters, with a letter and a number.'
    ...
    try {
      ...
      setFormError(err.response?.data?.message || 'That code is invalid or expired.'); setStatus('idle')
    }
  }
  ...
  return (
    <PartnerAuthShell
      ...
      subtitle={step === 'request' ? "We'll email you a 6-digit code to reset it." : `Enter the code sent to ${form.email} and choose a new password.`}
    ...
    >
      {step === 'request' ? (
        <form onSubmit={handleRequest} noValidate className="space-y-5">
          <Input id="email" name="email" type="email" label="Business email" value={form.email} onChange={handleChange} error={errors.email} placeholder="shop@yourlaundry.com" />
          {formError && <p role="alert" className="rounded-lg bg-red-50 px-4 py-3 text-sm font-medium text-red-600">{formError}</p>}
          <Button type="submit" variant="primary" className="w-full" disabled={status === 'pending'}>{status === 'pending' ? 'Sending…' : 'Send reset code'}</Button>
        </form>
      ) : (
        <form onSubmit={handleReset} noValidate className="space-y-5">
          ...
          {formError && <p role="alert" className="rounded-lg bg-red-50 px-4 py-3 text-sm font-medium text-red-600">{formError}</p>}
          <Button type="submit" variant="primary" className="w-full" disabled={status === 'pending'}>{status === 'pending' ? 'Updating…' : 'Reset password & sign in'}</Button>
        </form>
      )}
    </PartnerAuthShell>
  )
