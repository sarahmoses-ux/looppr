...
export default function AdminLoginVerify() {
  const { completeAdminLogin, resendAdminLoginCode } = useAuth()
  const navigate = useNavigate()
  const location = useLocation()
  const { challengeToken, email } = location.state || {}
  ...
  async function handleSubmit(e) {
    ...
    setStatus('pending')
    try {
      await completeAdminLogin(challengeToken, code)
  ...
  async function handleResend() {
    ...
    setResendMessage('')
    try {
      await resendAdminLoginCode(challengeToken)
