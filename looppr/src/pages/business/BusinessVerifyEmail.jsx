...
export default function BusinessVerifyEmail() {
  const { verifyEmail, resendVerification } = useBusinessAuth()
    ...
    >
      <form onSubmit={handleSubmit} noValidate className="space-y-5">
        {!location.state?.email && (
          <Input id="email" name="email" type="email" label="Business email" value={email} onChange={(e) => setEmail(e.target.value)} error={errors.email} placeholder="ops@yourbusiness.com" />
        )}
        <Input
          id="code"
          name="code"
        ...
        {resendNote && (
          <p className="rounded-lg bg-sky-50 px-4 py-3 text-sm font-medium text-sky-700">{resendNote}</p>
        )}
